...
def createCmap():
    ...
    for i in range (255):
        j = int(i % COLORS)
        colormap[i, 0, 0]=(round(125 * (np.sin((increment * j) + offset_r)+1)))
        colormap[i, 0, 1]=(round(125 * (np.sin((increment * j) + offset_b) +1)))
        colormap[i, 0, 2]=(round(125 * (np.sin((increment * j) + offset_g)+1)))
